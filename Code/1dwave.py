...
He simulates a wave as a discrete points on a string. 
He solves the wave equation numerically even though you could analytically solve this.
It works quiet well.
'''
...
pos = [0.0] * n # creates a python list that has n zeros
...
def increment():
    ...
    vel = [0.0] * n # create a python list of 0 velocity that is n in length
    ...
    for i in range(n):
        ...
        grad = ((get_bound(pos, i + 1) - get_bound(pos, i)) -
                (get_bound(pos, i) - get_bound(pos, i - 1))) / (dx**2) # finite central difference formula for second derivative
        
        # Calculate how much we need to adjust the velocity
        acc = c**2 * grad # propagation speed squared times by d2x/dt2
    ...
    if (total_time < osc_time):
        ...
        pos[0] = math.sin(total_time * freq) * amp
